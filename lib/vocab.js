...
exports.setpin = function(self, message, allDone) {
	//Test Relay
	let relay = (message.content.relay - 0) || false;
console.log(relay);
	if ((relay == false) || (relay > 3) || (relay < 0)) {
		allDone(true, 'Relay ID must be an integer from 0 to 3');
	}
	//Test Pin
	let pin = (message.content.pin - 0) || false;
console.log(pin);
	if (pin = false) {
		allDone(true, 'Pin must be an integer');
	};
	//Set and Save
